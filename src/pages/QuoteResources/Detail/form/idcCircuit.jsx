...
class OtherIdcCircuit extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isEditing: false,
  ...
  onPriceFormChange = (val) => {      //重要!!!!!
  	console.log(val,'PriceFormChange');
    this.form1.setValues(val);
    this.setState({
      priceForm: val
    })
  }
          ...
          <Row>
            ...
            <Col {...col6}>
              <Item
                {...formItemLayout}
                label="两端地址"
                dataIndex={`[${formListKey}].bothEnds`}
                required
                ...
                decorator={
                  checkEdit ?
                    ...
                    >
                      <Option value="杭州-杭州">杭州-杭州</Option>
                      <Option value="杭州-北京">杭州-北京</Option>
                    </Select>
                    :
                    <span>{_.get(updateForm, `[${formListKey}].bothEnds`, '')}</span>
                }
              />
            </Col>
            <Col {...col6}>
              <Item {...formItemLayout} label={"二级计费项"}
                ...
                decorator={
                  checkEdit ?
                    <Select mode='tags' optionFilterProp='children'
                      allowClear placeholder="请下拉选择或输入" >
                      {
                        autoList.map(item =>
                ...
                decorator={
                  checkEdit ?
                    <Select onFocus={() => this.getPriceUnit(formListKey)}
                      allowClear placeholder="请选择" mode='tags' optionFilterProp='children'>
                      { autoList.indexOf(itemFlag) !== -1 ?
...
export default connect(mapStateToProps, dispatchs('app', 'quoteResources'), null, { forwardRef: true })(OtherIdcCircuit)
