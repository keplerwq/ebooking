...
import { Input, Select, InputNumber, Row, Col, Cascader, DatePicker, Popover } from 'antd';
...
class ForeignResources extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isEditing: false,
  ...
  componentDidMount() {
    const { updateForm = {}, isEditing = false, formListKey,
      formListIndex, auth, provinceCityOptions,autoComplete } = this.props;
  ...
  render() {
    ...
    const { updateForm = {}, isEditing = false, formListKey,
      formListIndex, auth, provinceCityOptions, autoComplete,
      operatorsList } = this.props;
    const { priceFormList,formKey,fixForm,fixFormList,priceForm ,checkEdit} = this.state;
    /* 获取当前表单select的option */
      ...
      <div className="g-quote-from-res">
        ...
        >
          <Row>
            ...
            <Col {...col6}>
              <Item {...formItemLayout} label={"二级计费项"} required
                ...
                decorator={ checkEdit ?
                  <Select mode='tags' optionFilterProp='children' allowClear placeholder="请下拉选择或输入">
                    { autoList.map(item =>
            ...
            <Col {...col6}>
              <Item {...formItemLayout} label={"三级计费项"} required
                ...
                decorator={ checkEdit ?
                  ...
                  >
                    { // 二级计费项是否为自定义，若为自定义的话，则用自定义对应的列表来选择
                      autoList.indexOf(itemFlag) !== -1 ?
                        autoComplete[itemFlag] && autoComplete[itemFlag] && autoComplete[itemFlag].item3.map(item =>
                          <Option value={item} key={item} > {item} </Option>)
                        :
                        otherObj && otherObj.item3 && otherObj.item3.map(item => <Option value={item} key={item} > {item} </Option>)
                    }
                  </Select>
                  :
            ...
            <Col {...col6}>
              <Item {...formItemLayout} label="四级计费项" dataIndex={`[${formListKey}].item4`}
                required rules={[{ required: true, message: ' ' },]} trigger='onBlur'
                decorator={ checkEdit ?
                // <div style={{position: 'relative',width: '100%',lineHeight: '30px'}}>
          ...
          <Row>
            {
              (() => {
                switch (updateForm[formListKey].item2&&updateForm[formListKey].item2[0]){
                  case '机架': 
                  case 'u':
                  case '2u':
                  case '租机':
                  ...
                  case '端口':
                    return (
                      <>
                        <Col {...col6} key={1}>
                          <Item {...formItemLayout} label="机房地址" dataIndex={`[${formListKey}].roomAddress`}
                            rules={[{ required: true, message: ' ' }]} required
                            decorator={
                              checkEdit ? <Input maxLength={maxLength} placeholder="请填写机房地址" />
                                :
                                <span>{_.get(updateForm, `[${formListKey}].roomAddress`, '')}</span>
                            }
                          />
                        </Col>
                        <Col {...col6}>
                          <Item {...formItemLayout}></Item>
                        </Col>
                      </>
                    )
                  case '传输':
                    return (
                      <>
                        <Col {...col6} key={2}>
                          <Item {...formItemLayout} label="专线名称" dataIndex={`[${formListKey}].privateLineName`}
                            trigger='onBlur' required
                            rules={[{ required: true, message: ' ' },]}
                            decorator={
                              checkEdit ? 
                                <Input maxLength={maxLength} placeholder="请填写专线名称" />
                                :
                                <span>{_.get(updateForm, `[${formListKey}].privateLineName`, '')}</span>
                            }
                          />
                        </Col>
                        <Col {...col6}>
            ...
            <Col {...col6}>
              <Item {...formItemLayout} label="数量单位" 
                dataIndex={`[${formListKey}].unit`}
                required rules={[{ required: true, message: ' ' },{ type: 'array', len: 1, message: '请选择单个' }]}
                decorator={ checkEdit ?
                  <Select mode='tags' optionFilterProp='children' allowClear
                    placeholder="请下拉选择或输入"
                  >
            ...
            <Col {...col6}>
              <Item {...formItemLayout} label="币种" dataIndex={`[${formListKey}].currency`}
          ...
          { checkEdit ?
            <Row>
              {
                updateForm[formListKey].item2&&updateForm[formListKey].item2[0] == '传输' ? 
                  <Col {...col6}>
                    <Item {...formItemLayout} label="用量计费模式" dataIndex={`[${formListKey}].usageUnit`}
                      required rules={[{ required: true, message: ' ' },{ type: 'array', len: 1, message: '请选择单个' }]}
                      decorator={
                        <Select mode='tags' optionFilterProp='children' allowClear
                          placeholder="请下拉选择或输入" >
                          { autoComplete[`${updateForm[formListKey].item2}`] && autoComplete[`${updateForm[formListKey].item2}`].usageUnit.map(item =>
                            <Option value={item} key={item} >
                              {item}
                            </Option>)
                          }
                  ...
                  <Row>
                    <Col {...col6}>
                      <Item  {...formItemLayout}
                        ...
                        rules={[{ required: true, message: ' ' },]}
                        decorator={
                          <Cascader options={provinceCityOptions} placeholder="请选择" />
                        }
                      />
                    </Col>
                    <Col {...col6}>
                      <Item
                        ...
                        rules={[{ required: true, message: ' ' },]}
                        decorator={
                          <Input maxLength={maxLength} placeholder="国内请输地名，国外请输机房名称" />
                        }
                      />
                    </Col>
                    {
                      _.get(updateForm, `[${formListKey}].city`, [])[0] !== '海外' ?
                        <Col {...col6}>
                          <Item
                            key={0}
                            dataIndex={`[${formListKey}].operators`}
                            rules={[{ type: 'array', len: 1, message: '请选择单个' }]}
                            decorator={
                              <Select
                                allowClear
            ...
            <Row>
              <Col {...col6}>
                <Item
                  ...
                  decorator={
                    <span>{_.get(updateForm, `[${formListKey}].roomName`, '')}</span>
                  }
                />
              </Col>
              <Col {...col6}>
          ...
          <Row>
            <Col {...col6}>
              <Item {...formItemLayout} 
                ...
                dataIndex={`[${formListKey}].model`} required
                rules={[{ required: true, message: ' ' },]}
                decorator={ checkEdit ?
                  <Select allowClear placeholder="请选择" >
                    { autoList.indexOf(itemFlag) !== -1?
...
export default connect(mapStateToProps, dispatchs('app', 'quoteResources'), null, { forwardRef: true })(ForeignResources)
